            ...
            <div class="row justify-content-center">
                <div class="col-xxl-9 col-xl-10 col-lg-11">
                    ...
                    <div class="container">
                        <div class="row">
                            ...
                            <div class="col-lg-4 px-0 ps-lg-4 mb-4">
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <form method="get" action="{{ route('houses.reservations.input', ['house' => $house->id]) }}">
                                            <div class="form-group mb-2">
                                                <label class="col-form-label text-md-left fw-bold">チェックイン・チェックアウト</label>
                                                @error('fromCheckinDateToCheckoutDate')
                                                    <div class="text-danger small mb-2">{{ $message }}</div>
                                                @enderror
                                                <input type="text" class="form-control" name="fromCheckinDateToCheckoutDate" value="{{ old('fromCheckinDateToCheckoutDate') }}">
                                            </div>
                                            <div class="form-group mb-4">
                                                ...
                                                @error('numberOfPeople')
                                                    <div class="text-danger small mb-2">{{ $message }}</div>
                                                @enderror
                                                <input type="number" class="form-control" name="numberOfPeople" value="{{ old('numberOfPeople') ?? 1 }}" min="1">
                                            </div>
                    ...
                    <div>
                        ...
                        {{-- レビュー一覧カード/ページャ等はコントローラから$reviews, $userAll, $loginwas などをデータ渡し、下記JSへ --}}
                        <div id="cardContainer"></div>
                        {{-- ページネーション/一覧のJS（詳細は省略、@json($reviews)等でデータ渡し） --}}
                        <script>
                            // 例: window.reviews = @json($reviews);
                            // 必要に応じて上記JSロジックをここに
                        </script>
                    </div>
                </div>
